...
import { refreshTokenSignOptions, signJwtToken } from "../../lib/utils/jwt";
...
export class MfaService {
  public async generateMFASetup(req: Request) {
    ...
    if (user.userPreferences.enable2FA) {
      return { message: "User already has MFA enabled" };
    }

    let secretKey = user.userPreferences.twoFactorSecret;

    if (!secretKey) {
      const secret = speakeasy.generateSecret({ name: "Authenticator" });
